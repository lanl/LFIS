...
import numpy as np
from schedule import BridgeQuadraticSchedule

class LF_base(nn.Module):
    def __init__(self,ndim, flow, schedule,  target, prior, setup = 'transform', bridge = None, Bschedule = BridgeQuadraticSchedule()): 
        super().__init__()
        self.ndim = ndim
        self.flow = flow
    ...
    def comput_SandRHS(self,x, t):
        ...
        if self.bridge != None:
            logbridge = self.bridge.evaluate_log_density(x)
            dlogbridge = self.bridge.dlog_density(x)
            Boutf, Bdoutf = self.Bschedule(t)
            Score = Score + Boutf*dlogbridge
            RHS = RHS + (-Bdoutf * logbridge)
